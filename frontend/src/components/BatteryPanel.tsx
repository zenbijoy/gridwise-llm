...
export const BatteryPanel: React.FC<BatteryPanelProps> = ({ battery, onChangeBattery }) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Physical limits, rate boundaries, and reserve floors enforced deterministically by the LP solver.
          </p>
        </div>
      </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/60 rounded-xl p-4 border border-slate-200/80 dark:border-slate-700/80">
        <div className="flex items-center justify-between text-xs mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-800 dark:text-slate-200">State of Charge Gauge</span>
          </div>
          <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400 text-sm">
            {initialPct.toFixed(0)}% Initial SOC
          </span>
        </div>

        {/* Battery Container graphic */}
        <div className="relative w-full h-7 bg-slate-200 dark:bg-slate-700 rounded-lg overflow-hidden border border-slate-300 dark:border-slate-600">
          {/* Minimum Reserve Region */}
          <div
            className="absolute top-0 bottom-0 left-0 bg-rose-500/20 dark:bg-rose-500/30 border-r-2 border-rose-500 z-10"
            style={{ width: `${reservePct}%` }}
            title={`Minimum Reserve Floor: ${battery.minimum_energy_kwh} kWh (${reservePct.toFixed(0)}%)`}
          />

          {/* Current Initial Energy fill */}
          <div
            className="h-full bg-gradient-to-r from-emerald-500 to-teal-500 rounded-l-md transition-all duration-300"
            style={{ width: `${initialPct}%` }}
          />
        </div>

        {/* Legend / Key below bar */}
        <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 mt-2 font-mono">
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-sm bg-rose-500/40 border border-rose-500 inline-block" />
            <span>Min Reserve: {battery.minimum_energy_kwh} kWh ({reservePct.toFixed(0)}%)</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-sm bg-emerald-500 inline-block" />
            <span>Initial: {battery.initial_energy_kwh} kWh</span>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full text-xs font-mono font-medium px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
        </div>
